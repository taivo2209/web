	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "district":
			if(id_city != null) {
				...
				String textdictics = "<select id=\"iddistrict\" class=\"custom-select d-block w-100\" required onchange=\"load_town_ward(this.value);\" name=\"district\"> <option value=\"\"></option>"; 
				for (District ls : a.getQuanHuyenById(idCity)) {
		...
		case "town_ward":
			if(id_dictricst != null) {
				...
				String text_town_ward = "<select id=\"idtown_ward\" class=\"custom-select d-block w-100\" required onchange=\"showship();\" name=\"town_ward\"> <option value=\"\"></option>";
				for (Ward ls : a.getXaPhuongById(idDictricst)) {
